##############################################################################
# * Beginner's guid to Using Terraform on Azure
# 
...
resource "azurerm_resource_group" "tf_azure_nado" {
...
resource "azurerm_virtual_network" "vnet" {
  ...
  location            = "${azurerm_resource_group.tf_azure_nado.location}"
  address_space       = ["${var.address_space}"]
  resource_group_name = "${azurerm_resource_group.tf_azure_nado.name}"
}
...
resource "azurerm_network_security_group" "tf-nado-sg" {
  name                = "${var.prefix}-sg"
  location            = "${var.location}"
  resource_group_name = "${azurerm_resource_group.tf_azure_nado.name}"
...
resource "azurerm_network_interface" "tf-nado-nic" {
  name                      = "${var.prefix}tf-nado-nic"
  location                  = "${var.location}"
  resource_group_name       = "${azurerm_resource_group.tf_azure_nado.name}"
  network_security_group_id = "${azurerm_network_security_group.tf-nado-sg.id}"

  ip_configuration {
    name                          = "${var.prefix}ipconfig"
    subnet_id                     = "${azurerm_subnet.subnet.id}"
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = "${azurerm_public_ip.tf-nado-pip.id}"
  }
}
...
resource "azurerm_public_ip" "tf-nado-pip" {
  name                         = "${var.prefix}-ip"
  location                     = "${var.location}"
  resource_group_name          = "${azurerm_resource_group.tf_azure_nado.name}"
...
resource "azurerm_virtual_machine" "site" {
  ...
  network_interface_ids         = ["${azurerm_network_interface.tf-nado-nic.id}"]
  ...
  provisioner "file" {
    ...
    connection {
      ...
      host     = "${azurerm_public_ip.tf-nado-pip.fqdn}"
    }
  }
